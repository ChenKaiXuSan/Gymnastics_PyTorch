...
(``camera_position`` in ``configs/sam3d_triangulation.yaml``: four cameras on a
...
def draw_recovered_rigs(
    ...
    """
    from triangulation.camera_position_mapping import Extrinsics, draw_cameras_matplotlib
...
def assumed_relative_pose(cfg: Dict[str, Any]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    from triangulation.camera_position_mapping import (
        angles_to_position,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", default="logs/analysis/extrinsics/estimated_extrinsics.json"
    )
    parser.add_argument(
        "--camera-figure-dir",
        default=None,
