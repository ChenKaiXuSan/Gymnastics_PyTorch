...
# -*- coding:utf-8 -*-
"""
File: /workspace/VideoPose3D/run.py
Project: /workspace/VideoPose3D
Created Date: Sunday June 22nd 2025
Author: Kaixu Chen
-----
Comment:
...
-----
Last Modified: Sunday June 22nd 2025 10:28:28 pm
...
from videopose3d.common.loss import *
from videopose3d.common.generators import ChunkedGenerator, UnchunkedGenerator
from videopose3d.common.custom_dataset import CustomDataset
...
with initialize_config_dir(config_dir="/workspace/code/configs"):
...
dataset = CustomDataset(cfg.data.path + "filter_npz.npz")
...
keypoints = np.load(cfg.data.path + "filter_npz.npz", allow_pickle=True)
...
subjects_test = list(str(p) for p in Path(cfg.data.npz_path).rglob("*.npz"))
    ...
    for viz_subject in subjects_test:
        ...
        if (
            ...
            if "positions_3d" in dataset[args.viz_subject][args.viz_action]:
                ground_truth = dataset[args.viz_subject][args.viz_action][
                    "positions_3d"
        ...
        if args.viz_output is not None:
            ...
            viz_subject = Path(viz_subject)
            viz_video = Path(cfg.data.path) / "raw"
            viz_video = list(viz_video.rglob("*.MOV"))

            for i in viz_video:
                if (
                    viz_subject.stem in str(i)
                    and viz_subject.parent.stem in str(i)
                    and viz_subject.parent.parent.stem in str(i)
                ):
                    args.viz_video = i
                    break
            _output = (
