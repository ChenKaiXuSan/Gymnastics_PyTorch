...
from fusion.archive.rotation_aware import cli
from fusion.keypoints.config import load_skeleton_spec
from fusion.archive.rotation_aware.prefetch import ThroughputConfig
...
def test_twist_ablation_loss_and_model_flags_are_correct() -> None:
    from fusion.archive.rotation_aware.cli import loss_config_for_ablation, TWIST_ABLATIONS
...
def test_cross_attention_production_config_declares_equal_budgets() -> None:
    config = load_config("src/configs/archive/rotation_aware_cross_attention.yaml")
...
@pytest.mark.parametrize(
    "config_path",
    [
        "src/configs/archive/rotation_aware.yaml",
        "src/configs/archive/rotation_aware_batch64.yaml",
        "src/configs/archive/rotation_aware_cross_attention.yaml",
    ],
)
