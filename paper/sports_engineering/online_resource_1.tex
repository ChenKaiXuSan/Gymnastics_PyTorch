...
reported after one sequence-level similarity alignment to the same-video
triangulated pseudo-reference. Differences are method minus A6 with
participant-bootstrap 95\% confidence intervals and Holm-adjusted paired
Wilcoxon $p$ values. Rotation range of motion (ROM) and peak angular speed are
A3-relative retention ratios, not comparisons with independent kinematics.}
...
participants. Units are repository coordinates. The pseudo-reference-fitted
row is a leakage diagnostic and is not an eligible label-free method.}
\label{tab:deterministic}
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{p{0.47\linewidth}rcc}
\toprule
