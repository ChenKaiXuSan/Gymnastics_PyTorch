...
\input{generated/deterministic_comparison_all}
...
\section{Complete FreeMan zero-shot comparison}

The public FreeMan evaluation covered ten subjects (1, 7, 9, 12, 15, 19, 22,
29, 35, 36; 552 sessions) at frame stride 1 with all methods frozen. Values
are subject-balanced; Sim3 fits one similarity transform per session, PA is
per-frame Procrustes alignment. The pseudo-reference-fitted per-joint
weighting variant has no fitting target on FreeMan and is excluded as a
diagnostic. The FreeMan reference is a markerless multi-view reconstruction,
so all values measure agreement rather than absolute accuracy.

\begin{table}[h]
\caption{All FreeMan methods, subject-balanced over ten subjects and 552
sessions (mm). A6 is mean $\pm$ SD over training seeds 0--2.}
\label{tab:freeman-full}
\centering
\small
\begin{tabular}{lrr}
\toprule
...
A4, zero-shot & 364.363 & 103.114\\
A5, zero-shot & 364.218 & 103.199\\
...
\input{generated/joint_accuracy_all70}
\endgroup
