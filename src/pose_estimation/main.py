...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if "-h" in overrides or "--help" in overrides:
        print(
            "usage: python -m pose_estimation run [key=value ...]\n\n"
            "Run SAM3D-Body extraction using src/configs/pose_estimation/sam3d_body.yaml.\n"
            "Overrides use OmegaConf dot-list syntax, for example:\n"
            "  infer.person_list=[46] infer.gpu=[0] infer.workers_per_gpu=1"
        )
        return 0
    cfg = load_config(CONFIG_ROOT / "pose_estimation" / "sam3d_body.yaml", overrides)
    run(cfg)
    return 0
