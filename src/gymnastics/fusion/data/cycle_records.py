...
Cycle detection lives in :mod:`gymnastics.cycle_alignment`; this module only maps
...
from gymnastics.cycle_alignment.cycle_records import CycleRecord, cycle_record_path, read_cycle_record
...
def private_cycles_for_trials(
    ...
    *,
    ...
    if len(mids) != len(bounds):
        if require_mids:
            raise ValueError(f"{record_path} has no cycle middles; run `gymnastics align cycles private` first")
        mids = []
...
def public_cycles_for_sequence(
    ...
    *,
    ...
    Args:
        records_root: Root written by ``gymnastics align cycles <dataset>``.
    ...
    if not path.is_file():
        if require_record:
            raise FileNotFoundError(f"missing cycle record {path}; run `gymnastics align cycles` first")
        return (), (), None
