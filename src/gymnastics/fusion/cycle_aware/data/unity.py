...
Source dataset:
    ...
    SAM3D-Body is run on the rendered images by ``gymnastics benchmark unity``
...
Original skeleton:
    ...
    ``gymnastics.benchmarks.unity.mapping``) are scattered into MHR70 positions
...
def unity22_to_mhr70(points: np.ndarray, available: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    from gymnastics.benchmarks.unity.mapping import UNITY_JOINT_INDICES, _DIRECT
...
class UnityDataModule(DualViewDataModule):
    ...
    def _default_sequence_loader(self) -> SequenceLoader:
        ...
        def load() -> Sequence[tuple[PosePairTrial, np.ndarray | None, np.ndarray | None]]:
            ...
            from gymnastics.benchmarks.unity.fusion import build_pose_pair_trial
