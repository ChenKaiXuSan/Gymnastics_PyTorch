...
offline by :mod:`gymnastics.alignment.cycles` (``gymnastics align`` /
``gymnastics align cycles``) and read from record files; the training code
...
Phase normalisation (resampling):
    ...
    * without middles, the whole cycle is resampled at phases ``k / S``.
    ...
    :func:`gymnastics.fusion.cycle_aware.velocity.compute_velocity` yields
