...
Stages::
    ...
    gymnastics benchmark freeman-train prepare-cache --config configs/fusion/rotation_aware_freeman.yaml
    gymnastics benchmark freeman-train write-folds   --config ...
    gymnastics fuse rotation-aware train --config configs/fusion/rotation_aware_freeman.yaml \
        --fold configs/fusion/folds/freeman/fold_01.json --run-id freeman_fold01_a6_e12_s0 --ablation A6
    gymnastics benchmark freeman-train evaluate --config ... --seed 0
    gymnastics benchmark freeman-train compare  --config ... --seed 0
...
from gymnastics.fusion.rotation_aware.cli import load_config as load_rotation_config
...
DEFAULT_CONFIG = "configs/fusion/rotation_aware_freeman.yaml"
...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="gymnastics benchmark freeman-train")
    ...
    for name in ("prepare-cache", "write-folds", "plan", "evaluate", "baselines", "zero-shot", "compare"):
        ...
        if name == "zero-shot":
            ...
            child.add_argument("--rotation-config", default="configs/fusion/rotation_aware.yaml")
