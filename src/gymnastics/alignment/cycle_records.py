...
Cycle detection runs once, offline, in :mod:`gymnastics.alignment` and
writes one JSON record per sequence.  Training code
(:mod:`gymnastics.fusion.cycle_aware`) only reads these files through
...
Private recordings (``local/runs/split_cycle/person_<id>/alignment_record_<id>.json``)::
    ...
    ``start``/``end`` are written by ``gymnastics align``; ``mid`` and
    ``metadata.cycle_detection`` are added by ``gymnastics align cycles
    private`` (or by a fresh ``gymnastics align`` run).  Frame numbers refer
...
class CycleRecord:
    ...
    def spans(self) -> Tuple[CycleSpan, ...]:
        ...
        if not self.has_mids:
            raise ValueError("record has no middle frames; run `gymnastics align cycles` first")
