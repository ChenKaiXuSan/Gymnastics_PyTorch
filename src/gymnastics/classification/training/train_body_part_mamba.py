...
from gymnastics.classification.models.skeleton_mamba import build_skeleton_mamba_model
...
class BodyPartMambaClassificationModule(LightningModule):
    ...
    def _unpack_batch(batch: Dict[str, Any]):
        if isinstance(batch, dict):
            ...
            raise TypeError(
                f"Unsupported batch type: {type(batch)}. Expected dict batch from gymnastics.classification.data."
            )
