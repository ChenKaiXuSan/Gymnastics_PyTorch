...
from gymnastics.classification.models.ssm import build_ssm_classifier_from_hparams
from gymnastics.classification.utils.helper import save_helper
...
class SSMTrainer(LightningModule):
    ...
    def _compute_and_log_loss(
        ...
        for task in self.tasks:
            ...
            acc = metric_group["accuracy"].to(task_logits.device)(
                task_preds, task_label_valid
            )
            precision = metric_group["precision"].to(task_logits.device)(
                task_preds, task_label_valid
            )
            recall = metric_group["recall"].to(task_logits.device)(
                task_preds, task_label_valid
            )
