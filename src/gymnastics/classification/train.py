...
from gymnastics.common.config import load_config
from gymnastics.common.paths import CONFIG_ROOT
from gymnastics.classification.data.datamodule import PersonDataModule
from gymnastics.classification.map_config import PersonInfo

#####################################
# select different experiment trainer
#####################################

from gymnastics.classification.training.train_stgcn import STGCNTrainer
from gymnastics.classification.training.train_ssm import SSMTrainer
from gymnastics.classification.training.train_tcn import TCNTrainer
...
def train(hparams: DictConfig, dataset_idx, fold: int):
    ...
    if trainer_name == "st_gcn":
        ...
        from gymnastics.classification.training.train_body_part_mamba import BodyPartMambaClassificationModule
...
def run_config(config: DictConfig) -> None:
    #########
    # K fold
    #########
...
def init_params(argv: Sequence[str] | None = None) -> int:
    overrides = list(sys.argv[1:] if argv is None else argv)
    if "-h" in overrides or "--help" in overrides:
        print(
            "usage: gymnastics classify [key=value ...]\n\n"
            "Train classifiers using configs/classification/train.yaml."
        )
        return 0
    config = load_config(CONFIG_ROOT / "classification" / "train.yaml", overrides)
    run_config(config)
    return 0


if __name__ == "__main__":
    raise SystemExit(init_params())
