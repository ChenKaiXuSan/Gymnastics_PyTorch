...
from gymnastics.classification.splits.cross_validation_camera_pairs import CameraPairCrossValidation
...
def hydra_main(cfg: Optional[DictConfig] = None) -> None:
    ...
    默认读取 configs/classification/train.yaml 中 data.cross_validation 字段，
    也支持命令行 override，例如：
    ...
    generate_index_files(
        ...
        num_persons=int(cv_cfg.get("num_persons", 2)),
        num_actions=int(cv_cfg.get("num_actions", 12)),
        num_cameras=int(cv_cfg.get("num_cameras", 108)),
        strategies=list(cv_cfg.get("strategies", ["by_person"])),
        n_splits=int(cv_cfg.get("n_splits", 5)),
        force_recreate=bool(cv_cfg.get("force_recreate", False)),
