...
:mod:`fusion.data.windows` turns samples into the
...
Common Coordinate System:
    ...
    *pelvis-centred body frame* (``fusion.keypoints.geometry``):
...
Cycle Information:
    ...
    ``python -m cycle_alignment align`` / ``python -m cycle_alignment cycles``; the training code
...
def _mhr70_spec():
    ...
    return load_skeleton_spec(CONFIG_ROOT / "shared" / "skeleton_mhr70.yaml")
...
def canonicalize_view(points: np.ndarray, valid: np.ndarray) -> tuple[np.ndarray, np.ndarray, CanonicalTransformRecord]:
    ...
    """
    from fusion.keypoints.geometry import canonicalize_pose
