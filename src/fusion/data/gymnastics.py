...
Cycle information:
    Exact, read from ``alignment_record_<id>.json`` (``python -m cycle_alignment align``
    writes the boundaries, ``python -m cycle_alignment cycles private`` adds the
...
Split:
    The fixed paper protocol ``src/configs/shared/folds/paper_137_a6_split.json``
...
class GymnasticsDataModule(DualViewDataModule):
    ...
    def _default_trial_loader(self) -> TrialLoader:
        ...
        if source == "raw":
            ...
            def load_raw(person_id: str) -> Sequence[PosePairTrial]:
                ...
                from fusion.keypoints.data import load_person_trials

                spec = load_skeleton_spec(CONFIG_ROOT / "shared" / "skeleton_mhr70.yaml")
    ...
    def _person_ids(self) -> list[str]:
        ...
        fold = json.loads(_resolve(self.config.options.get("fold_json", "src/configs/shared/folds/paper_137_a6_split.json")).read_text(encoding="utf-8"))
