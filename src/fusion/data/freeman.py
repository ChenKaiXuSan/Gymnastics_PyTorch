...
Source dataset:
    ...
    (``python -m fusion benchmark-freeman``), which selected two views per session.
...
Cycle information:
    ...
    ``python -m cycle_alignment cycles freeman`` and read from
...
def coco17_to_mhr70(points: np.ndarray, valid: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray]:
    ...
    from fusion.benchmarks.freeman.mapping import _MHR_INDICES
...
class FreeManDataModule(DualViewDataModule):
    ...
    def _default_session_loader(self) -> SessionLoader:
        ...
        def load(subject: int) -> Sequence[tuple[PosePairTrial, Path | None]]:
            from fusion.benchmarks.freeman.fusion import build_rotation_aware_trial
