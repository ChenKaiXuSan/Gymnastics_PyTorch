...
def _paths(config: Mapping[str, Any], output_override: str | None) -> dict[str, Path]:
    ...
    return {
        ...
        "output": root,
        "cache": cache_root,
        "skeleton": Path(paths.get("skeleton", "src/configs/shared/skeleton_mhr70.yaml")),
    }
...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m fusion rotation-aware")
    parser.add_argument("--config", default="src/configs/archive/rotation_aware.yaml")
