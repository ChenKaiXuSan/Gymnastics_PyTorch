"""Offline cycle and middle annotation for every dataset (``python -m cycle_alignment cycles``).
...
(:mod:`cycle_alignment.cycles`, right-wrist azimuth in the body frame).

Sub-commands::

    python -m cycle_alignment cycles private [--log-root local/runs/split_cycle] [--kpt-root ...] [--person 1 2]
        Adds ``mid`` (turn-around frame) to the existing
        ``alignment_record_<id>.json`` files of the private recordings.  The
        cycle boundaries written by ``python -m cycle_alignment align`` are kept unchanged;
        only the middle is computed from the two SAM3D views of each cycle.

    python -m cycle_alignment cycles freeman [--benchmark-root local/runs/freeman_benchmark_cluster] [--subjects 12 13]
    ...
    python -m cycle_alignment cycles unity [--benchmark-root ...] [--sam3d-cache-root ...]
...
def fused_wrist_theta(
    ...
    *,
    ...
    This reproduces the signal ``python -m cycle_alignment align`` segments on: each view is
...
def run_freeman(args: argparse.Namespace) -> int:
    from fusion.benchmarks.freeman.training import load_manifest_pair, load_manifest_sessions
...
Generated by `python -m cycle_alignment cycles index` on {timestamp}. One directory per
...
## Definition (shared by every dataset, `src/cycle_alignment/cycles.py`)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m cycle_alignment cycles", description="Offline cycle and middle annotation.")
