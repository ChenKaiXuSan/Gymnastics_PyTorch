...
cycle" are defined.  It is used by ``python -m cycle_alignment align`` for the private
recordings and by ``python -m cycle_alignment cycles`` for the public datasets, so
every dataset shares one definition.  Training code never calls it; it reads
the records written by :mod:`cycle_alignment.cycle_records`.
...
def smooth_1d(x: np.ndarray, win: int = DEFAULT_SMOOTH_WINDOW) -> np.ndarray:
    """Odd-window moving average with edge padding (same as ``python -m cycle_alignment align``)."""
    win = max(3, int(win) | 1)
    pad = win // 2
