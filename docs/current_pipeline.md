...
- 三角化的相机内参来自棋盘格标定，外参由 `triangulation.estimate_extrinsics` 从数据估计。两视角几何是无尺度的，重投影误差无法反映基线长度是否正确；伪真值的米制尺度来自 SAM3D 单目 3D，未经器械标定。
- 融合指标默认使用 `similarity` 对齐（逐序列拟合含尺度的 Sim3），因此**方法排序和相对比较对伪真值的尺度误差完全免疫**；只有绝对毫米值与尺度误差成正比。报告绝对精度时应同时给出 `analysis/normalize_by_body_scale.py` 产出的无量纲指标（误差占体长百分比），该指标与尺度无关。
