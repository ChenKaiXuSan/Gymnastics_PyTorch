#!/usr/bin/env python3
"""Consolidate the regenerated-GT fusion results into logs/fuse_experiments.
...
RAE_EVAL = Path(
    "logs/fuse_rotation_aware/evaluation/"
...
def main() -> None:
    ...
    if RAE_EVAL.exists():
        ...
        A("On this pseudo-GT MPJPE, the learned network is on par with — marginally "
          ...
          "See `logs/fuse_rotation_aware/evaluation/.../{report.json,rotation_metrics.csv}`.")
    else:
        A("_Network evaluation not found; run `python -m fuse.rotation_aware evaluate "
    ...
    A("- To pin absolute scale: the recordings are on a matted hall floor whose tatami "
      ...
      "reference. See `analysis/normalize_by_body_scale.py` and "
    ...
    A(f"**Reconstruction plausibility** — stature median {st['median']:.2f} m "
      ...
      "adults). Full report: `logs/analysis/triangulated_results/triangulated_results_report.md`; "
