...
#   bash pegasus/submit_cycle_aware_5fold.sh freeman                    # sweep freeman_v1_5fold
...
set -eu
DATA="${1:?usage: submit_cycle_aware_5fold.sh <gymnastics|freeman> [experiment]}"
EXPERIMENT="${2:-}"
SEED="${SEED:-0}"
EPOCHS="${EPOCHS:-50}"
SWEEP="${SWEEP:-${DATA}_v1${EXPERIMENT:+_$EXPERIMENT}_5fold${SEED:+_seed$SEED}}"
case "$DATA" in
